...
{ // Pra testar esse aqui eu preciso olhar a memória
    ...
    cpu->write(address, 0b1111);

    cpu->executeInstruction( incBC );

    REQUIRE(cpu->read(address) == 0b10000); // Simple one, the third bit doesn't change
...
TEST_CASE("LD [n16], SP instruction working", "[ld]") {
    ...
    *cpu->get16bitRegister(RegistersEnum::PC) = addressPC; //Tem que ser em algum pedaço da memória que possa ser lido
    ...
    REQUIRE(cpu->read(address) == (value & 0xFF));
    REQUIRE(cpu->read(address + 1) == (value >> 8));
